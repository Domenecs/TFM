...
public class SimpleRodLineEnd : MonoBehaviour
{
    ...
    [SerializeField]
    private SimpleRodManager refSimpleRodManager;
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        if (other.CompareTag("Water"))
        {
            ...
            if (refSimpleRodManager != null)
            {
                if (elapsedTime >= randomTime && !refSimpleRodManager.IsFishHooked)
                {
                    Debug.Log("Calling SpinningManager");
                    elapsedTime = 0;
                    StartCoroutine(refSimpleRodManager.FishingSequence(iterationCount));
                    iterationCount++;
